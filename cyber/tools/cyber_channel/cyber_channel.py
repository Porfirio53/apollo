...
class CyberChannelecho(object):
    def __init__(self, channel_name, mqtt_broker="127.0.0.1", mqtt_port=1883, mqtt_topic="apollo", mqtt_user="user1", mqtt_password="password1"):
    ...
    def callback(self, raw_data):
        """
        Channel echo callback, now also sends data over MQTT.
        """
        # 获取 msgtype
        msgtype = cyber.ChannelUtils.get_msgtype(self.channel_name, 0).decode('utf-8')

        # 将指定 Channel 数据存储至 Msg 中
        Msg = cyber.ChannelUtils.get_debugstring_rawmsgdata(msgtype, raw_data).decode('utf-8')

        # 打印 Msg
        # print(Msg)

        # 打印成功提示
        print("Msg received and sent to MQTT Broker")

        # 将 Msg 发布到 MQTT Broker
        self.client.publish(self.mqtt_topic, Msg)

    def start_listening(self):
        """
        Start listening to the channel and sending data via MQTT.
        """
        node_echo = cyber.Node("listener_node_echo")
        node_echo.create_rawdata_reader(self.channel_name, self.callback)
...
def channel_echo(channel_name, mqtt_broker="127.0.0.1", mqtt_port=1883, mqtt_topic="apollo"):
    """
    Reader message and sends over MQTT.
    """
    echo_cb = CyberChannelecho(channel_name, mqtt_broker, mqtt_port, mqtt_topic)
    echo_cb.start_listening()

def _channel_cmd_echo(argv):
    """
    Command-line parsing for 'cyber_channel echo' command with MQTT option.
    """
    args = argv[2:]
    from optparse import OptionParser
    parser = OptionParser(
    ...
    # 调用 channel_echo 函数进行监听和 MQTT 发布
    channel_echo(args[0])
